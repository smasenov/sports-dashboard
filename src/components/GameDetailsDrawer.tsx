      ...
      <>
        ...
        <GradientBackground>
          <Grid container spacing={3} alignItems="center">
            ...
            <Grid item xs={5} textAlign="center">
              <TeamLogo src={getTeamLogo(awayTeam?.team)} alt={awayTeam?.team.displayName} />
            ...
            {/* Home Team */}
            <Grid item xs={5} textAlign="center">
              <TeamLogo src={getTeamLogo(homeTeam?.team)} alt={homeTeam?.team.displayName} />
              <Typography variant="h6" fontWeight={600} mt={1} color="white">
        ...
        <TabPanel value={tabValue} index={0}>
          {data.leaders && data.leaders.length > 0 ? (
            <Grid container spacing={2}>
              {(data.leaders as unknown as TeamLeadersData[]).map((teamLeaders, teamIdx) => (
                <Grid item xs={12} key={teamIdx}>
                  {teamLeaders.leaders?.map((statCategory: StatCategory, statIdx: number) => (
                    <Card key={statIdx} sx={{ mb: 2 }}>
                      <CardContent>
                        <Typography variant="h6" gutterBottom>
                          {statCategory.displayName}
                        </Typography>
                        {statCategory.leaders?.map((item: PlayerLeader, itemIdx: number) => (
                          <Box key={itemIdx} mb={2}>
                            <Grid container spacing={2} alignItems="center">
                              <Grid item xs={2}>
                                <TeamLogo
                                  src={getTeamLogo(teamLeaders.team)}
                                  alt={teamLeaders.team?.displayName || 'Team'}
                                  style={{ width: 40, height: 40 }}
                                />
                              </Grid>
                              <Grid item xs={6}>
                                <Typography variant="body1" fontWeight={600}>
                                  {item.athlete?.displayName || 'Unknown'}
                                </Typography>
                                <Typography variant="caption" color="text.secondary">
